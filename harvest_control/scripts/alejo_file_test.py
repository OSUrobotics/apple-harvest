...
from harvest_interfaces.srv import TextInRviz, BackAndForth

class AlejoTrial(Node):
    def __init__(self):
        super().__init__('alejo_trial')

        # Create services (type, name, function)
        self.text_in_rviz_srv = self.create_service(TextInRviz, 'place_text_in_rviz', self.toy_problem_callback)
        self.graps_apple_srv = self.create_service(GraspApple, 'grasp_apple', self.grasp_apple_callback)
